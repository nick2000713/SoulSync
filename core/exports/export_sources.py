...
_MATCHED_TRACK_SQL = """
    SELECT t.musicbrainz_id, t.spotify_id, t.external_ids, t.id,
...
def _db_match(artist: str, title: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    ``track_id`` is the lib2 track the repair jobs key their findings on, used to
    cross-check the finding the mbid_mismatch job may already have on file for it."""
